SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |menu|
    menu.item :booking, 'Booking Invitation', private_bookings_path, icon: 'icon-invitation', if: proc { current_user.role?(:artist) }
    ...
    menu.item :payments, 'Payments', private_payments_path, icon: 'icon-pay', if: proc { current_user.role?(:artist) }
